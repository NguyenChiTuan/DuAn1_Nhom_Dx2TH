...
import com.javaweb.service.LoaiTinTucService;
...
public class ThemTinServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Date date = new Date();
        int loaitin = Integer.parseInt(request.getParameter("QLDMM"));
        String iduser = request.getParameter("iduser");
        try {
            Tintuc tt = new Tintuc();
            Loaitintuc LT = LTTService.GetLoaiTinTheoId(loaitin);
            Users us = USService.GetUsersByID(iduser);
            tt.setImage(image);
            tt.setTieuDe(tieude);
            tt.setNoiDung(noidung);
            tt.setTomTatTin(tomtat);
            tt.setNgayDang(date);
            tt.setLoaitintuc(LT);
            tt.setUsers(us);
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ThemTinServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ThemTinServlet at " + request.getContextPath() + image+ tomtat+noidung+loaitin+iduser+ "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
